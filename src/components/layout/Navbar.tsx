      ...
      <header style={{
          ...
          <nav className="nav-desktop" style={{ alignItems: "center", gap: 28 }}>
            {navLinks.map((link) => (
              <Link key={link.label} href={link.href}
                style={{ color: "rgba(255,255,255,0.8)", fontFamily: "DM Sans, sans-serif", fontSize: 14, fontWeight: 500, textDecoration: "none" }}>
                {link.label}
              </Link>
            ))}
          </nav>
        ...
        {mobileOpen && (
          <div style={{ background: "rgba(2,26,26,0.98)", borderTop: "1px solid rgba(255,255,255,0.08)", padding: "16px 20px 24px", display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <Link href={`/${locale}/planner`} onClick={() => setMobileOpen(false)}
