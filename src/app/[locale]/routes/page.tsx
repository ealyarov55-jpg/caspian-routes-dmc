...
import { MapPin, Clock, Star, Users, Filter, Search } from "lucide-react";
...
export default function RoutesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  useEffect(() => {
    getDocs(collection(db, "providers")).then(snap => {
      const data = snap.docs.map(d => d.data() as Provider);
      setProviders(data);
  ...
  const availableProviders = providers.filter(p =>
    p.availableDates?.some(d => d >= today)
  );
    ...
    <div style={{ minHeight: "100vh", background: "#f0f7f7", fontFamily: "DM Sans, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(160deg, #021a1a 0%, #065050 100%)", padding: "80px 24px 60px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 16, maxWidth: 500, marginBottom: 32 }}>
            Choose your journey and connect with local drivers & guides
          </p>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "48px 24px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 32, alignItems: "start" }}>
          ...
          {/* Routes */}
          <div>
            <p style={{ color: "#4a6060", fontSize: 13, marginBottom: 20 }}>{filtered.length} routes found</p>
                ...
                <div key={route.id}
                  ...
                  <div style={{ display: "flex" }}>
                    <div style={{ width: 200, flexShrink: 0, position: "relative" }}>
                      <img src={route.image} alt={route.title} style={{ width: "100%", height: "100%", objectFit: "cover", minHeight: 140 }} />
                      {route.tag && (
                    ...
                    <div style={{ padding: "20px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", borderLeft: "1px solid #f0f7f7" }}>
                      ...
                      <span style={{ fontSize: 11, color: "#94a3a3" }}>
                        {availableProviders.length} guide{availableProviders.length !== 1 ? "s" : ""} available
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div style={{ position: "sticky", top: 24 }}>
            <div style={{ background: "white", borderRadius: 20, padding: 24, boxShadow: "0 4px 24px rgba(4,46,46,0.08)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
                ...
                <h3 style={{ fontFamily: "Cormorant Garamond, serif", fontSize: 20, color: "#021a1a", fontWeight: 600 }}>Available Guides</h3>
              </div>
              ...
              {loadingProviders ? (
                ...
                <div style={{ textAlign: "center", padding: "20px 0" }}>
                  <p style={{ color: "#94a3a3", fontSize: 13 }}>No providers available yet</p>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {availableProviders.map(p => (
                    <div key={p.uid} style={{ padding: 16, borderRadius: 14, border: "1.5px solid #e2eded", transition: "all 0.2s" }}>
